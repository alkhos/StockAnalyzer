...
def get_annuals(symbol, api_key): 
    ...
    r = requests.get('https://finnhub.io/api/v1/stock/financials-reported?token=' + api_key + '&freq=quarterly' +'&symbol=' + symbol) 
    res = r.json()
...
def save_as_json(finiancial_statements):
    ...
    with open('quarterly_financial_' + symbol + '.json', 'w') as data_output_file:
